...
bool selfSpatialReuseDetected = false;
...
// ### SELF-SPATIAL REUSE FUNCTIONS ###
static void analyzeArrayReference(raw_ostream &OS, Instruction *I,
                               ArrayRef<Loop*> LoopNest,
                               ScalarEvolution *SE) {

    // Get the pointer operand of the load/store instruction
    Value *Ptr = getLoadStorePointerOperand(I);
    if (!Ptr)
        return;

    const SCEV *InstSCEV = SE->getSCEV(Ptr);
    const SCEVAddRecExpr *AddRec = dyn_cast<SCEVAddRecExpr>(InstSCEV);

    // Get step value for each iteration of the inner loop
    const SCEV *Step = AddRec->getStepRecurrence(*SE);
    const SCEVConstant *StepConstant = dyn_cast<SCEVConstant>(Step);

    // Get Element Size in bytes
    const SCEV *ElemSize = SE->getElementSize(I);
    const SCEVConstant *ElemSizeConst = dyn_cast<SCEVConstant>(ElemSize);
    int64_t ElementSize = ElemSizeConst->getAPInt().getSExtValue();

    // Calculate Elements per cache line for a given CacheLineSize
    int64_t ElementsPerCacheLine = CacheLineSize / ElementSize;

    // Calculate Stride of the reuse
    int64_t Stride = StepConstant->getAPInt().getSExtValue() / ElementSize;

    // Detect self-spatial reuse
    // Check if the stride value is within the elements per cache line
    if (Stride <= ElementsPerCacheLine) {
        selfSpatialReuseDetected = true;

        // Find self-spatial reuse level
        unsigned Level = LoopNest.size();
        for (unsigned i = 0; i < LoopNest.size(); ++i) {
          if (!isa<SCEVAddRecExpr>(SE->getSCEVAtScope(Ptr, LoopNest[i]))) {
            Level = i + 1;
            break;
          }
        }

        // Print reference, reuse level and stride
        OS << *I << "\n";
        OS << "level: " << Level << "\n";
        OS << "stride: " << Stride << "\n";
    }
...
static void analyzeSpatialReuse(raw_ostream &OS, ScalarEvolution &SE, LoopInfo &LI) {
  ...
  // Get all loops in preorder
  auto LoopNest = LI.getLoopsInPreorder();

  // Get the inner most loop
  Loop *InnerLoop = LoopNest.back();

  // Get the basic block for the inner loop
  // Assume there's only one basic block
  BasicBlock *InnerLoopBB = InnerLoop->getBlocks()[0];

  // Loop through the instructions in the inner loop basic block
  for (auto &I : *InnerLoopBB) {
    // Analyze array references for each instruction in the inner loop
    analyzeArrayReference(OS, &I, LoopNest, &SE);
  }

  if (!selfSpatialReuseDetected)
    OS << "No self-spatial reuse detected.\n";

  OS << "\n";
}

// ### END OF SELF-SPATIAL REUSE FUNCTIONS ###
